...
% %%
% Chl_a_seasons_one_degree_res = zeros(46,15,51); 
% for i = 1:51
%     
%  Chl_a_seasons_one_degree_res(:,:,i) = BlockMean(Chl_a_seasons(:,:,i),...
...
% 
% %%
% Chl_a_winter_one_degree = zeros(46,15,39);
% 
% for i = 1 : 39
%     
%  Chl_a_winter_one_degree(:,:,i) = BlockMean(Chl_a_winter(:,:,i),...
%         (length(Longitude_subset) ./ length(step_Lon)) ,...
%         length(Latitude_subset)  ./ length(step_Lat)); 
% end
% 
% Chl_a_winter_one_degree = rot90(Chl_a_winter_one_degree); 
% 
% 
% %%
% Chl_a_spring_one_degree = zeros(46,15,39);
% 
% for i = 1:39 
%     
%  Chl_a_spring_one_degree(:,:,i) = BlockMean(Chl_a_spring(:,:,i),...
%      (length(Longitude_subset) ./ length(step_Lon)) ,...
%         length(Latitude_subset)  ./ length(step_Lat));
% end
% 
% Chl_a_spring_one_degree = rot90(Chl_a_spring_one_degree) ; 
% 
% %%
% Chl_a_summer_one_degree = zeros(46,15,37); 
% 
% for i = 1:37 
%     Chl_a_summer_one_degree(:,:,i) = BlockMean(Chl_a_summer(:,:,i),...
%      (length(Longitude_subset) ./ length(step_Lon)) ,...
%         length(Latitude_subset)  ./ length(step_Lat));
% end
% 
% Chl_a_summer_one_degree = rot90(Chl_a_summer_one_degree) ; 
% 
% %%
% 
% Chl_a_fall_one_degree = zeros(46,15,36) ; 
% 
% for i = 1:36 
%     
%  Chl_a_fall_one_degree(:,:,i) = BlockMean(Chl_a_fall(:,:,i),...
%      (length(Longitude_subset) ./ length(step_Lon)) ,...
%         length(Latitude_subset)  ./ length(step_Lat));
% end
% 
% Chl_a_fall_one_degree = rot90(Chl_a_fall_one_degree); 
% %%
% 
% 
% %% saving CMOD, Ice, and Winds in monthly grid 
% 
...
%             % statement in here
%             
%             eval(sprintf('Lat_CMOD = Total_timetable_CMOD(TR_%s_%d,:).Total_Latitude_Night_Cloud_Free;', MONTH{j},i))
%             eval(sprintf('Lon_CMOD = Total_timetable_CMOD(TR_%s_%d, :).Total_Longitude_Night_Cloud_Free;',MONTH{j}, i))
%             eval(sprintf('OD       = Total_timetable_CMOD(TR_%s_%d, :).CMOD;', MONTH{j}, i))
%             eval(sprintf('Lat_Ice  = Total_timetable_Depol_Ratio(TR_%s_%d,:).Total_Latitude_Ice;', MONTH{j}, i))
%             eval(sprintf('Lon_Ice  = Total_timetable_Depol_Ratio(TR_%s_%d,:).Total_Longitude_Ice;', MONTH{j}, i))
%             eval(sprintf('Ice      = Total_timetable_Depol_Ratio(TR_%s_%d,:).Total_Surface_532_Integrated_Depolarization_Ratio;', MONTH{j},i))
%             eval(sprintf('Lat_Wind = Total_timetable_amsrmf(TR_%s_%d,:).Total_Latitude_Wind;', MONTH{j}, i))
%             eval(sprintf('Lon_Wind = Total_timetable_amsrmf(TR_%s_%d, :).Total_Longitude_Wind;', MONTH{j},i))
%             eval(sprintf('amsrMF   = Total_timetable_amsrmf(TR_%s_%d,:).Total_windamsrMF;', MONTH{j},i))
%             
%            
...
%             [SeaIce, SeaIce_OCC, SeaIce_STD, SeaIce_ERR] = hist_wt_occ_tot(Lat_Ice_nonan, Lon_Ice_nonan, Ice_nonan, step_Lat', step_Lon');
%             [Winds, Winds_OCC, Winds_STD, Winds_ERR]     = hist_wt_occ_tot(Lat_Wind, Lon_Wind, amsrMF, step_Lat', step_Lon');
%             
